    ...
    <header className="sticky top-0 z-40 flex h-14 items-center gap-4 border-b bg-card px-4 lg:px-6">
      ...
      <div className="flex flex-1 items-center gap-4">
        <div className="relative max-w-md flex-1">
          ...
          <Input
            placeholder="Search contacts, events, or type a query..."
          ...
          <DropdownMenuContent align="end" className="w-80">
            ...
            {selfReminders.length > 0 ? (
              selfReminders.map((reminder) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    <span className={`h-2 w-2 rounded-full ${reminder.type === 'warning' ? 'bg-warning' : 'bg-info'}`} />
            ...
            <DropdownMenuSeparator />
            <DropdownMenuItem className="justify-center text-primary cursor-pointer" onClick={() => navigate('/reminders')}>
              View all reminders
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
